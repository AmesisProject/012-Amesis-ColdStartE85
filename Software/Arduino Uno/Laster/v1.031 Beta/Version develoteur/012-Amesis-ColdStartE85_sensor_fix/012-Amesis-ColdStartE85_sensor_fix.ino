...
int DTC_AT ;                     // Defaut de la sonde de temperature
 ...
 degre();
  ...
  //float resistance; // Le float sert pour le microcontroleur du modul LAPX9C10X_X9C103
         if (optionValue  == 4){ //Si Option 4 est enclanchée 
               ...
               if (airTempValue <= 0) { counter = 69 ; }          //sinon on met au max de l'option 3 
               ...
               if (airTempValue <= 0) { counter = 39 ; }         //sinon on met au max de l'option 2 
...
void degre (){// Cette fonction n'est pas exploitée pour le moment 
  ...
  airTempValue = T - 273.15 + ATOffSet;
...
void DTC(){
       if (airTempValue == ATMin) {DTC_AT = 1 ;} //Si la valeur min à été atteinte alors defaut 
  else if (airTempValue == ATMax) {DTC_AT = 1 ;} //Si la valeur max à été atteinte alors defaut 
  else {DTC_AT = 0 ;}                            //Sinon pas de defaut
